...
- [x] **L14: `config.js`のSupabase認証情報がハードコード**
  - ファイル: `js/config.js` (L2-3)
  - 内容: `SUPABASE_URL`と`SUPABASE_ANON_KEY`がlocalhostにハードコード
  - 修正案: 環境変数から読み込むか、ビルド時に置換する仕組みを導入
  - **保留: デプロイ時に対応**
- [x] **L16: DOM要素の重複取得**
  - ファイル: `js/judge.js`
  - 内容: `getSideElems()`が呼ばれるたびに`querySelector()`を実行
  - 修正案: 初期化時に一度だけ取得してキャッシュする
  - **済: 2026-03-13 `redElems` / `whiteElems` をキャッシュ**
- [x] **L17: `parseOrZero`の型安全性不足**
  - ファイル: `js/judge.js` (L11-15)
  - 内容: 空文字列や`null`を0として扱うが、未選択状態との区別がつかない
  - 修正案: 明示的に`null`を返すバージョンを作成
  - **保留: M6で入力検証済みのため優先度低**

## 🆕 2026-03-13 追加修正

- [x] **M15: スコアボードモード切替時に再描画されない**
  - ファイル: `js/admin-utils.js` (L9-14)
  - 内容: `setScoreboardMode()`でモードを変更するが、`loadData()`を呼ばないため即座に切り替わらない
  - **済: 2026-03-13 イベントリスナーを `admin-core.js` に移動し、モード変更後に `loadData(false)` を呼び出し**

- [x] **M16: 音声再生中の`patchState`エラーハンドリング不足**
  - ファイル: `js/admin-audio.js` (L97, 115, 124)
  - 内容: `patchState()`の失敗を`.catch()`で握りつぶすだけで、ユーザーに通知しない
  - **済: 2026-03-13 エラー時に警告メッセージを表示し、上書きしないよう修正**

- [x] **M17: `console.log`が本番環境に残存**
  - ファイル: `js/judge.js` (L414)
  - 内容: 審査員の採点内容がブラウザコンソールに漏洩
  - **済: 2026-03-13 削除**

- [x] **M18: エラーハンドリングで具体的なエラー情報が欠落**
  - ファイル: 複数のEdge Functions
  - 内容: クライアントには`{error: "..."}`の汎用メッセージのみ返す
  - **済: 2026-03-13 Edge Functions で `detail` フィールドを追加、クライアント側で表示**
