...
- ✅ `user-guide.md`: 利用者向けの確定仕様
- ✅ `developer-guide.md`: 実装・運用の確定仕様
- ✅ `future-improvements.md`: 改善バックログと検討メモ（暫定案を含む）

### 運用ルール

- このファイルは削除せず、今後も「検討中」「保留」「あとで調べる」を残す。
- 項目ごとに日付と状態（案/検討中/保留/決定）を付ける。
- 確定した仕様は `user-guide.md` / `developer-guide.md` に反映し、このファイルには要約とリンクのみ残す。

---

## 2. current_match=false 時の進行操作を禁止する

> 2026-05-11: 実装済み。`js/admin-core.js` と `js/admin-utils.js` で、current match 以外を表示中のときは `受付開始/停止`、`E5`、`E6`、`Epoch設定` を無効化する。
...
> 2026-07-08: Phase 1（OBSスコアボード）は着手済み。`obs-scoreboard.js` / `obs-scoreboard-vertical.js` は Realtime 購読で `state`、現在試合の `submissions` / `expected_judges` / `matches` 変更を検知し、既存ポーリングをフォールバックとして残す構成にした。DB側は `20260708000002_enable_obs_realtime.sql` で対象テーブルを `supabase_realtime` publication に追加する。
...
2. ✅ `obs-scoreboard-vertical.js` でRealtime購読を実装
3. ✅ DB側のRealtime publication設定マイグレーションを追加
4. ローカル環境で挙動テスト
5. 本番環境で試験運用
...
## 2026-05-10 追記: 大会概念と連携方針

- 大会概念（`tournaments`）導入、トークンprefixの大会単位管理、kuawase連携の方向性は別紙に記録。
- 参照: `docs/tournament-direction-2026-05-10.md`

---

## 2026-07-08 追記: 審査員TOKEN再発行時のQR表示

**状態:** 保留

審査員TOKENは平文保存せず、作成・再発行直後だけ管理画面に表示する方針とする。
現在は再発行後にTOKENと審査員URLを一度だけ表示するが、現場運用ではスマートフォンで読み込めるQR表示があると配布が楽になる。

将来対応する場合は以下の方針が望ましい。

- 再発行成功レスポンスに含まれるURLから、その場でQRを生成する
- DBにはTOKEN平文やQR画像を保存しない
- QR生成ライブラリを使う場合はローカル同梱またはSRI付きCDNにする
- 印刷・拡大表示・コピー用URLを同じ画面に置く
- 再発行後に画面を閉じると再表示できないことを明示する
